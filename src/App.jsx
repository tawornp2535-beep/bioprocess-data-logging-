...
function App() {
  ...
  const [refJobId, setRefJobId] = useState(''); // Reference Batch ID
  const [showReportModal, setShowReportModal] = useState(false); // PDF Report Preview Modal
  ...
  const SURVEY_QUESTIONS = [
    { id: 'q1', section: 'กระบวนการให้บริการ', short: 'ขั้นตอนการให้บริการ', text: 'ขั้นตอนการให้บริการมีความเหมาะสม คล่องตัว เข้าใจง่าย' },
    { id: 'q2', section: 'กระบวนการให้บริการ', short: 'ช่องทางการติดต่อ', text: 'มีช่องทางการติดต่อที่สะดวก เหมาะสม เช่น โทรศัพท์ เว็บไซต์ เครือข่ายสังคมออนไลน์' },
    { id: 'q3', section: 'กระบวนการให้บริการ', short: 'ระยะเวลาให้บริการ', text: 'ระยะเวลาในการให้บริการเหมาะสม' },
    { id: 'q4', section: 'เจ้าหน้าที่ผู้ให้บริการ', short: 'บุคลิกภาพเจ้าหน้าที่', text: 'เจ้าหน้าที่ผู้ให้บริการมีบุคลิกภาพที่ดี และใส่ใจในการให้บริการ' },
    { id: 'q5', section: 'เจ้าหน้าที่ผู้ให้บริการ', short: 'คำแนะนำของเจ้าหน้าที่', text: 'เจ้าหน้าที่ผู้ให้บริการให้คำแนะนำและตอบข้อซักถามอย่างชัดเจน' },
    { id: 'q6', section: 'เจ้าหน้าที่ผู้ให้บริการ', short: 'ความรู้เจ้าหน้าที่', text: 'เจ้าหน้าที่ผู้ให้บริการมีความรู้ความสามารถในการให้บริการ' },
    { id: 'q7', section: 'การประชาสัมพันธ์', short: 'สื่อประชาสัมพันธ์', text: 'สื่อประชาสัมพันธ์ เช่น โบรชัวร์ มีการแจ้งรายละเอียดข้อมูลของเครื่องมืออย่างชัดเจนและครอบคลุม' },
    { id: 'q8', section: 'การประชาสัมพันธ์', short: 'ช่องทาง PR', text: 'มีการประชาสัมพันธ์หลากหลายช่องทาง' },
    { id: 'q9', section: 'สถานที่และสิ่งอำนวยความสะดวก', short: 'สถานที่ให้บริการ', text: 'สถานที่ให้บริการมีความเหมาะสมและเข้าถึงได้สะดวก' },
    { id: 'q10', section: 'สถานที่และสิ่งอำนวยความสะดวก', short: 'สิ่งอำนวยความสะดวก', text: 'สถานที่รอรับบริการมีสิ่งอำนวยความสะดวกเพียงพอ เช่น ที่นั่ง' },
    { id: 'q11', section: 'คุณภาพการให้บริการ', short: 'คุณภาพบริการ', text: 'ผลิตภัณฑ์หรือบริการของเราตอบสนองต่อความคาดหวังของคุณได้ดีเพียงใด' },
    { id: 'q12', section: 'ระบบติดตามและบันทึกข้อมูล (SCADA DBMS)', short: 'ระบบติดตามเรียลไทม์', text: 'ระบบติดตามข้อมูลเรียลไทม์ (Real-time Dashboard) ช่วยให้ท่านเห็นสถานะการทำงานได้อย่างสะดวก รวดเร็ว' },
    { id: 'q13', section: 'ระบบติดตามและบันทึกข้อมูล (SCADA DBMS)', short: 'โหมดนำเสนอและกราฟ', text: 'โหมดการนำเสนอข้อมูล (Presentation Mode) และกราฟวิเคราะห์มีความสวยงาม ชัดเจน และใช้งานง่าย' },
  ];
  ...
  const refJob = React.useMemo(() => {
    return jobs.find(j => j.id === refJobId) || null;
  }, [jobs, refJobId]);

  const refDataMapped = React.useMemo(() => {
    if (!refJob || !refJob.data) return [];
    return mapDataPointsToLiters(refJob.data, refJob).sort((a, b) => {
      const aTime = a.timestamp ? new Date(a.timestamp).getTime() : 0;
      const bTime = b.timestamp ? new Date(b.timestamp).getTime() : 0;
      return aTime - bTime;
    });
  ...
  const mergedChartData = React.useMemo(() => {
    if (!refJob || refDataMapped.length === 0) return chartData;
    
    // Map refData by rounded cultureHour (to 1 decimal place)
    const refMap = {};
    refDataMapped.forEach(row => {
      const hr = Math.round(row.cultureHour * 10) / 10;
      refMap[hr] = row;
    });

    return chartData.map(row => {
      const hr = Math.round(row.cultureHour * 10) / 10;
      const refRow = refMap[hr];
      if (refRow) {
        return {
          ...row,
          temp_read_ref: refRow.temp_read,
          ph_read_ref: refRow.ph_read,
          do_read_ref: refRow.do_read,
          agit_read_ref: refRow.agit_read,
          air_read_ref: refRow.air_read
        };
      }
      return row;
    });
  }, [chartData, refJob, refDataMapped]);

  const reportStats = React.useMemo(() => {
    if (!currentJob || !chartData || chartData.length === 0) {
      return {
        temp: { min: 0, max: 0, avg: 0 },
        ph: { min: 0, max: 0, avg: 0 },
        do: { min: 0, max: 0, avg: 0 },
        agit: { min: 0, max: 0, avg: 0 },
        air: { min: 0, max: 0, avg: 0 }
      };
    }
    ...
    keys.forEach(k => {
      const vals = chartData.map(d => d[k]).filter(v => typeof v === 'number' && !isNaN(v));
      const keyName = k.replace('_read', '');
      if (vals.length > 0) {
        const min = Math.min(...vals);
        const max = Math.max(...vals);
        const avg = vals.reduce((a, b) => a + b, 0) / vals.length;
        stats[keyName] = { min, max, avg };
      } else {
        stats[keyName] = { min: 0, max: 0, avg: 0 };
      }
    });

    return stats;
  }, [currentJob, chartData]);
  ...
  const handleExportFeedback = () => {
    ...
    const headers = [
      ...
      'q11_คุณภาพบริการ',
      'q12_ระบบติดตามเรียลไทม์',
      'q13_โหมดนำเสนอและกราฟ',
      'ช่องทางที่รู้จัก',
  ...
  const exportToPDF = () => {
    ...
    const savedUser = localStorage.getItem('bioprocess-user') || '';
    setReportOperator(savedUser || 'Operator');
    setReportNotes('');
  ...
  return (
    <>
      {/* Print-only Batch Report */}
      <div id="batch-report-print" style={{ display: 'none' }}>
        <div style={{ padding: '20mm', fontFamily: 'Inter, "IBM Plex Sans Thai", sans-serif', color: '#000000', backgroundColor: '#ffffff' }}>
          {/* Report Header */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid #0f172a', paddingBottom: '12px', marginBottom: '20px' }}>
            <div>
              <h1 style={{ fontSize: '24px', fontWeight: 800, margin: 0, color: '#0f172a' }}>BIOPROCESS BATCH REPORT</h1>
              <p style={{ fontSize: '12px', color: '#475569', margin: '4px 0 0 0' }}>SCADA Database Management System (DBMS)</p>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: '14px', fontWeight: 700, color: '#0f172a' }}>{aboutSystem?.systemName || 'DBMS'}</div>
              <div style={{ fontSize: '11px', color: '#64748b' }}>Version {aboutSystem?.systemVersion || 'v2.4.0'}</div>
            </div>
          </div>

          {/* Batch Summary Card */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '25px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '16px' }}>
            <div>
              <p style={{ margin: '4px 0', fontSize: '13px' }}><strong>Batch Name:</strong> {currentJob?.name || '-'}</p>
              <p style={{ margin: '4px 0', fontSize: '13px' }}><strong>Vessel / Machine:</strong> {currentMachine?.name || '-'}</p>
              <p style={{ margin: '4px 0', fontSize: '13px' }}><strong>Operator:</strong> {reportOperator || '-'}</p>
            </div>
            <div>
              <p style={{ margin: '4px 0', fontSize: '13px' }}><strong>Start Date/Time:</strong> {currentJob?.createdAt ? new Date(currentJob.createdAt).toLocaleString('th-TH') : '-'}</p>
              <p style={{ margin: '4px 0', fontSize: '13px' }}><strong>Print Date:</strong> {new Date().toLocaleString('th-TH')}</p>
              <p style={{ margin: '4px 0', fontSize: '13px' }}><strong>Total Samples logged:</strong> {chartData.length} points</p>
            </div>
          </div>

          {/* Statistics Grid */}
          <h3 style={{ fontSize: '16px', fontWeight: 700, marginBottom: '10px', color: '#0f172a', borderBottom: '1px solid #e2e8f0', paddingBottom: '6px' }}>Key Parameter Statistics</h3>
          <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '30px' }}>
            <thead>
              <tr style={{ background: '#f1f5f9' }}>
                <th style={{ border: '1px solid #cbd5e1', padding: '8px 12px', fontSize: '12px', textAlign: 'left' }}>Parameter</th>
                <th style={{ border: '1px solid #cbd5e1', padding: '8px 12px', fontSize: '12px', textAlign: 'center' }}>Minimum (PV)</th>
                <th style={{ border: '1px solid #cbd5e1', padding: '8px 12px', fontSize: '12px', textAlign: 'center' }}>Maximum (PV)</th>
                <th style={{ border: '1px solid #cbd5e1', padding: '8px 12px', fontSize: '12px', textAlign: 'center' }}>Average (PV)</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td style={{ border: '1px solid #cbd5e1', padding: '8px 12px', fontSize: '12px', fontWeight: 600 }}>Temperature (°C)</td>
                <td style={{ border: '1px solid #cbd5e1', padding: '8px 12px', fontSize: '12px', textAlign: 'center' }}>{reportStats?.temp?.min?.toFixed(2) || '0.00'}</td>
                <td style={{ border: '1px solid #cbd5e1', padding: '8px 12px', fontSize: '12px', textAlign: 'center' }}>{reportStats?.temp?.max?.toFixed(2) || '0.00'}</td>
                <td style={{ border: '1px solid #cbd5e1', padding: '8px 12px', fontSize: '12px', textAlign: 'center' }}>{reportStats?.temp?.avg?.toFixed(2) || '0.00'}</td>
              </tr>
              <tr>
                <td style={{ border: '1px solid #cbd5e1', padding: '8px 12px', fontSize: '12px', fontWeight: 600 }}>pH Level</td>
                <td style={{ border: '1px solid #cbd5e1', padding: '8px 12px', fontSize: '12px', textAlign: 'center' }}>{reportStats?.ph?.min?.toFixed(2) || '0.00'}</td>
                <td style={{ border: '1px solid #cbd5e1', padding: '8px 12px', fontSize: '12px', textAlign: 'center' }}>{reportStats?.ph?.max?.toFixed(2) || '0.00'}</td>
                <td style={{ border: '1px solid #cbd5e1', padding: '8px 12px', fontSize: '12px', textAlign: 'center' }}>{reportStats?.ph?.avg?.toFixed(2) || '0.00'}</td>
              </tr>
              <tr>
                <td style={{ border: '1px solid #cbd5e1', padding: '8px 12px', fontSize: '12px', fontWeight: 600 }}>Dissolved Oxygen (%)</td>
                <td style={{ border: '1px solid #cbd5e1', padding: '8px 12px', fontSize: '12px', textAlign: 'center' }}>{Math.round(reportStats?.do?.min || 0)}</td>
                <td style={{ border: '1px solid #cbd5e1', padding: '8px 12px', fontSize: '12px', textAlign: 'center' }}>{Math.round(reportStats?.do?.max || 0)}</td>
                <td style={{ border: '1px solid #cbd5e1', padding: '8px 12px', fontSize: '12px', textAlign: 'center' }}>{Math.round(reportStats?.do?.avg || 0)}</td>
              </tr>
              <tr>
                <td style={{ border: '1px solid #cbd5e1', padding: '8px 12px', fontSize: '12px', fontWeight: 600 }}>Agitation speed (RPM)</td>
                <td style={{ border: '1px solid #cbd5e1', padding: '8px 12px', fontSize: '12px', textAlign: 'center' }}>{Math.round(reportStats?.agit?.min || 0)}</td>
                <td style={{ border: '1px solid #cbd5e1', padding: '8px 12px', fontSize: '12px', textAlign: 'center' }}>{Math.round(reportStats?.agit?.max || 0)}</td>
                <td style={{ border: '1px solid #cbd5e1', padding: '8px 12px', fontSize: '12px', textAlign: 'center' }}>{Math.round(reportStats?.agit?.avg || 0)}</td>
              </tr>
              <tr>
                <td style={{ border: '1px solid #cbd5e1', padding: '8px 12px', fontSize: '12px', fontWeight: 600 }}>Air Flow (L/min)</td>
                <td style={{ border: '1px solid #cbd5e1', padding: '8px 12px', fontSize: '12px', textAlign: 'center' }}>{reportStats?.air?.min?.toFixed(1) || '0.0'}</td>
                <td style={{ border: '1px solid #cbd5e1', padding: '8px 12px', fontSize: '12px', textAlign: 'center' }}>{reportStats?.air?.max?.toFixed(1) || '0.0'}</td>
                <td style={{ border: '1px solid #cbd5e1', padding: '8px 12px', fontSize: '12px', textAlign: 'center' }}>{reportStats?.air?.avg?.toFixed(1) || '0.0'}</td>
              </tr>
            </tbody>
          </table>

          {/* Report Notes / Conclusion */}
          {reportNotes && (
            <div style={{ marginBottom: '30px', pageBreakInside: 'avoid' }}>
              <h3 style={{ fontSize: '16px', fontWeight: 700, marginBottom: '10px', color: '#0f172a', borderBottom: '1px solid #e2e8f0', paddingBottom: '6px' }}>Notes & Experimental Conclusions</h3>
              <div style={{ fontSize: '12px', lineHeight: 1.6, color: '#334155', background: '#f8fafc', border: '1px solid #e2e8f0', padding: '12px', borderRadius: '6px', whiteSpace: 'pre-wrap' }}>
                {reportNotes}
              </div>
            </div>
          )}

          {/* Batch Records Table Summary */}
          <div style={{ pageBreakInside: 'avoid' }}>
            <h3 style={{ fontSize: '16px', fontWeight: 700, marginBottom: '10px', color: '#0f172a', borderBottom: '1px solid #e2e8f0', paddingBottom: '6px' }}>Batch Data Logs (Sampling Points)</h3>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#f1f5f9' }}>
                  <th style={{ border: '1px solid #cbd5e1', padding: '6px 8px', fontSize: '11px', textAlign: 'center' }}>ชั่วโมงที่</th>
                  <th style={{ border: '1px solid #cbd5e1', padding: '6px 8px', fontSize: '11px', textAlign: 'center' }}>วัน/เวลา</th>
                  <th style={{ border: '1px solid #cbd5e1', padding: '6px 8px', fontSize: '11px', textAlign: 'center' }}>TEMP PV</th>
                  <th style={{ border: '1px solid #cbd5e1', padding: '6px 8px', fontSize: '11px', textAlign: 'center' }}>pH PV</th>
                  <th style={{ border: '1px solid #cbd5e1', padding: '6px 8px', fontSize: '11px', textAlign: 'center' }}>DO PV</th>
                  <th style={{ border: '1px solid #cbd5e1', padding: '6px 8px', fontSize: '11px', textAlign: 'center' }}>AGIT PV</th>
                  <th style={{ border: '1px solid #cbd5e1', padding: '6px 8px', fontSize: '11px', textAlign: 'center' }}>AIR PV</th>
                  <th style={{ border: '1px solid #cbd5e1', padding: '6px 8px', fontSize: '11px', textAlign: 'left' }}>บันทึกเพิ่มเติม (Remarks)</th>
                </tr>
              </thead>
              <tbody>
                {chartData.map((row, idx) => (
                  <tr key={idx}>
                    <td style={{ border: '1px solid #cbd5e1', padding: '6px 8px', fontSize: '10px', textAlign: 'center' }}>{row.cultureHour.toFixed(1)}</td>
                    <td style={{ border: '1px solid #cbd5e1', padding: '6px 8px', fontSize: '10px', textAlign: 'center' }}>{row.date} {row.time}</td>
                    <td style={{ border: '1px solid #cbd5e1', padding: '6px 8px', fontSize: '10px', textAlign: 'center' }}>{row.temp_read.toFixed(2)}</td>
                    <td style={{ border: '1px solid #cbd5e1', padding: '6px 8px', fontSize: '10px', textAlign: 'center' }}>{row.ph_read.toFixed(2)}</td>
                    <td style={{ border: '1px solid #cbd5e1', padding: '6px 8px', fontSize: '10px', textAlign: 'center' }}>{Math.round(row.do_read)}</td>
                    <td style={{ border: '1px solid #cbd5e1', padding: '6px 8px', fontSize: '10px', textAlign: 'center' }}>{Math.round(row.agit_read)}</td>
                    <td style={{ border: '1px solid #cbd5e1', padding: '6px 8px', fontSize: '10px', textAlign: 'center' }}>{row.air_read.toFixed(1)}</td>
                    <td style={{ border: '1px solid #cbd5e1', padding: '6px 8px', fontSize: '10px', textAlign: 'left' }}>{row.remark || '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div style={{ marginTop: '50px', display: 'flex', justifyContent: 'space-between', pageBreakInside: 'avoid' }}>
            <div style={{ width: '200px', borderTop: '1px solid #94a3b8', textAlign: 'center', paddingTop: '8px', fontSize: '11px', color: '#475569' }}>
              ลงชื่อผู้รายงาน (Operator Signature)
            </div>
            <div style={{ width: '200px', borderTop: '1px solid #94a3b8', textAlign: 'center', paddingTop: '8px', fontSize: '11px', color: '#475569' }}>
              ลงชื่อผู้ตรวจสอบ (Supervisor Signature)
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="feedbacks-view">
            ...
            {(() => {
              ...
              const sectionDefs = [
                ...
                { name: 'ด้านระบบติดตามและบันทึกข้อมูล (SCADA DBMS)', keys: ['q12', 'q13'] },
              ];
                      ...
                      {fb.scores && (
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '4px' }}>
                          {Object.entries(fb.scores).map(([k, v]) => {
                            const qInfo = SURVEY_QUESTIONS.find(q => q.id === k);
                            const label = qInfo ? qInfo.short : k;
                ...
                <div className="glass-panel" style={{ height: '650px', padding: '2rem' }}>
                  <div className="chart-header" style={{ marginBottom: '1.5rem', flexDirection: 'column', alignItems: 'flex-start', gap: '1rem' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', width: '100%', padding: '0.5rem', background: 'rgba(15, 23, 42, 0.3)', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                      ...
                      {/* Reference Batch Selector Dropdown */}
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginLeft: 'auto', padding: '2px 0' }}>
                        <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', fontWeight: 600 }}>📈 เทียบกับ Batch อ้างอิง:</span>
                        <select
                          value={refJobId}
                        ...
                        >
                          <option value="">-- ไม่เลือก (None) --</option>
                          {jobs
                            .filter(j => j.id !== currentJob?.id && j.data && j.data.length > 0)
                            .map(j => {
                              const machine = machines.find(m => m.id === j.machineId);
                              const machineLabel = machine ? ` [${machine.name}]` : '';
                              return (
                  ...
                  <ResponsiveContainer width="100%" height="80%">
                    <LineChart data={mergedChartData} margin={{ top: 25, right: 10, left: 10, bottom: 5 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.1)" />
                      <XAxis 
                        dataKey="cultureHour" 
                      ...
                      {/* Reference Batch Lines */}
                      {refJobId && visibleParameters.temp && (
                        <Line yAxisId="temp" type="monotone" dataKey="temp_read_ref" name={`Ref TEMP (${refJob?.name})`} stroke="var(--accent-red)" strokeWidth={2} strokeDasharray="4 4" opacity={0.4} dot={false} connectNulls={true} />
                      )}
                      {refJobId && visibleParameters.ph && (
                        <Line yAxisId="ph" type="monotone" dataKey="ph_read_ref" name={`Ref pH (${refJob?.name})`} stroke="var(--accent-blue)" strokeWidth={2} strokeDasharray="4 4" opacity={0.4} dot={false} connectNulls={true} />
                      )}
                      {refJobId && visibleParameters.do && (
                        <Line yAxisId="do" type="monotone" dataKey="do_read_ref" name={`Ref DO (${refJob?.name})`} stroke="var(--accent-green)" strokeWidth={2} strokeDasharray="4 4" opacity={0.4} dot={false} connectNulls={true} />
                      )}
                      {refJobId && visibleParameters.agit && (
                        <Line yAxisId="agit" type="monotone" dataKey="agit_read_ref" name={`Ref AGIT (${refJob?.name})`} stroke="var(--accent-yellow)" strokeWidth={2} strokeDasharray="4 4" opacity={0.4} dot={false} connectNulls={true} />
                      )}
                      {refJobId && visibleParameters.air && (
                        <Line yAxisId="air" type="monotone" dataKey="air_read_ref" name={`Ref AIR (${refJob?.name})`} stroke="var(--accent-purple)" strokeWidth={2} strokeDasharray="4 4" opacity={0.4} dot={false} connectNulls={true} />
                      )}
      ...
      {showReportModal && (
        <div className="modal-backdrop" onClick={() => setShowReportModal(false)}>
          <div className="modal-container" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '960px', width: '95vw', padding: '1.5rem' }}>
            <div className="modal-header">
              <h3>
                <Database size={22} color="var(--accent-purple)" />
                พรีวิวรายงานประวัติ Batch (Batch Report Export)
              </h3>
              <button className="modal-close-btn" onClick={() => setShowReportModal(false)} title="ปิด">
                <X size={20} />
              </button>
            </div>
            <div className="modal-body" style={{ flexDirection: 'row', flexWrap: 'wrap', gap: '1.5rem', maxHeight: '75vh', overflowY: 'auto' }}>
              
              {/* Left Column: Form Controls */}
              <div style={{ flex: '1 1 350px', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div style={{ background: 'rgba(15, 23, 42, 0.2)', border: '1px solid var(--border-color)', borderRadius: '10px', padding: '1rem' }}>
                  <h4 style={{ margin: '0 0 1rem 0', fontSize: '0.9rem', color: 'var(--text-primary)' }}>📝 ข้อมูลเอกสารรายงาน</h4>
                  
                  <div className="form-group" style={{ marginBottom: '1rem' }}>
                    <label className="modal-label">ชื่อผู้ควบคุม (Operator Name)</label>
                    <input 
                      type="text" 
                  ...
                  <div className="form-group">
                    <label className="modal-label">บันทึกเพิ่มเติม / สรุปผลการทดลอง (Notes & Conclusion)</label>
                    <textarea 
                      className="modal-input" 
                      style={{ minHeight: '120px', resize: 'vertical', fontFamily: 'inherit' }}
                      value={reportNotes} 
                      onChange={(e) => setReportNotes(e.target.value)} 
                      placeholder="ระบุข้อแนะนำ บันทึกเหตุการณ์ หรือสรุปผลเพิ่มเติม..."
                    />
                  </div>
                </div>
              ...
              <div style={{ flex: '1 1 500px', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <span className="modal-label" style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>📄 ตัวอย่างหน้ากระดาษพิมพ์จริง (A4 Print Preview):</span>
                <div style={{ 
                  background: 'white', 
                  color: 'black', 
                  borderRadius: '8px', 
                  ...
                  {/* Mini-Render of Print Header */}
                  <div style={{ borderBottom: '2px solid #0f172a', paddingBottom: '6px', marginBottom: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div>
                      <h4 style={{ margin: 0, fontSize: '13px', fontWeight: 800, color: '#0f172a' }}>BIOPROCESS BATCH REPORT</h4>
                      <p style={{ margin: 0, fontSize: '9px', color: '#64748b' }}>SCADA Database Management System (DBMS)</p>
                    </div>
                    <div style={{ textAlign: 'right', fontSize: '9px', color: '#64748b' }}>
                      <strong>{aboutSystem?.systemName || 'DBMS'}</strong>
                    </div>
                  </div>

                  {/* Summary Block */}
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', fontSize: '9px', background: '#f8fafc', padding: '8px', border: '1px solid #e2e8f0', borderRadius: '4px', marginBottom: '12px' }}>
                    <div>
                      <strong>Batch Name:</strong> {currentJob?.name || '-'}<br/>
                      <strong>Vessel/Machine:</strong> {currentMachine?.name || '-'}<br/>
                      <strong>Operator:</strong> {reportOperator || '-'}
                    </div>
                    <div>
                      <strong>Start Time:</strong> {currentJob?.createdAt ? new Date(currentJob.createdAt).toLocaleString('th-TH', { dateStyle: 'short', timeStyle: 'short' }) : '-'}<br/>
                      <strong>Print Date:</strong> {new Date().toLocaleDateString('th-TH')}<br/>
                      <strong>Total Logs:</strong> {chartData.length} points
                    </div>
                  </div>

                  {/* Mini Stats Table */}
                  <h5 style={{ margin: '0 0 4px 0', fontSize: '10px', fontWeight: 700 }}>Key Parameter Statistics</h5>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '8px', marginBottom: '12px' }}>
                    <thead>
                      <tr style={{ background: '#f1f5f9' }}>
                        <th style={{ border: '1px solid #cbd5e1', padding: '3px 4px', textAlign: 'left' }}>Parameter</th>
                        <th style={{ border: '1px solid #cbd5e1', padding: '3px 4px', textAlign: 'center' }}>Min</th>
                        <th style={{ border: '1px solid #cbd5e1', padding: '3px 4px', textAlign: 'center' }}>Max</th>
                        <th style={{ border: '1px solid #cbd5e1', padding: '3px 4px', textAlign: 'center' }}>Avg</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr>
                        <td><strong>Temp (°C)</strong></td>
                        <td style={{ textAlign: 'center' }}>{reportStats.temp.min.toFixed(1)}</td>
                        <td style={{ textAlign: 'center' }}>{reportStats.temp.max.toFixed(1)}</td>
                        <td style={{ textAlign: 'center' }}>{reportStats.temp.avg.toFixed(1)}</td>
                      </tr>
                      <tr>
                        <td><strong>pH</strong></td>
                        <td style={{ textAlign: 'center' }}>{reportStats.ph.min.toFixed(2)}</td>
                        <td style={{ textAlign: 'center' }}>{reportStats.ph.max.toFixed(2)}</td>
                        <td style={{ textAlign: 'center' }}>{reportStats.ph.avg.toFixed(2)}</td>
                      </tr>
                      <tr>
                        <td><strong>DO (%)</strong></td>
                        <td style={{ textAlign: 'center' }}>{Math.round(reportStats.do.min)}</td>
                        <td style={{ textAlign: 'center' }}>{Math.round(reportStats.do.max)}</td>
                        <td style={{ textAlign: 'center' }}>{Math.round(reportStats.do.avg)}</td>
                      </tr>
                    </tbody>
                  </table>
                  ...
                  {reportNotes && (
                    <div style={{ fontSize: '9px', marginBottom: '10px' }}>
                      <h5 style={{ margin: '0 0 3px 0', fontWeight: 700 }}>Notes</h5>
                      <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', padding: '6px', borderRadius: '4px', whiteSpace: 'pre-wrap', color: '#334155' }}>
                        {reportNotes}
                      </div>
                    </div>
                  )}
                  
                  <span style={{ fontSize: '8px', color: '#64748b', fontStyle: 'italic', display: 'block', textAlign: 'center', marginTop: '10px' }}>
                    * หน้าพรีวิวย่อ - หน้ากระดาษพิมพ์จริงจะมีตารางข้อมูลตัวอย่างและพื้นที่ลงนามเพิ่มเต็มใบ *
                  </span>
                </div>
              </div>
